...
function filterStudentsByGrade(students, grade) {
  ...
  const filteredGrade = students.filter((key) => key.grade === grade);
  filteredGrade.forEach((studentName) => console.log(`${studentName.name}`));
...
function warnAboutMaxArrayLength(arr, maxLength) {
  ...
  } else console.info("Довжина масиву не перевищує максимально допустиму.");
...
compareArrays([4, 1, 3, 2, 5], [1, 2, 3, 4]);
...
function calculateTotalPrice(products) {
  ...
  const total = products.reduce((accum, item) => (accum += item.price), 0);
...
function countVowelsAndConsonants(word) {
  ...
  for (let letter of word) {
    let smallLatter = letter.toLowerCase();
    if (vowels.includes(smallLatter)) {
      console.count("vowels");
    } else if (smallLatter.match(/[a-z]/i)) {
